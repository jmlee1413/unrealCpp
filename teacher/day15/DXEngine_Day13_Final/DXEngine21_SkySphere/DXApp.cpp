...
DXApp::DXApp(HINSTANCE hinstance)
{
	...
	clientWidth = 1600;
	clientHeight = 960;
	applicationName = L"Engine21 - Sky Sphere";
	...
	camera.SetAspecRatio(clientWidth, clientHeight);
}
...
void DXApp::InitMeshInfo()
{
	...
	Texture cubeMap;
	cubeMap.fileName 
		= L"Resources//Textures//skybox2.dds";

	Texture cubeMap1;
	cubeMap1.fileName
		= L"Resources//Textures//SaintPetersBasilica.dds";
	...
	Mesh cube(fbxNameCube, L"Shaders//SkyboxVS.fx", L"Shaders//SkyboxPS.fx");
	cube.AddTexture(cubeMap1);
	cube.SetPosition(XMFLOAT3(450.0f, 0.0f, 0.0f));
	cube.SetScale(XMFLOAT3(100.0f, 100.0f, 100.0f));
	...
	plane.AddTexture(planeMap);
	plane.SetPosition(XMFLOAT3(0.0f, -90.0f, 0.0f));
	plane.SetRotation(XMFLOAT3(-90.0f, 0.0f, 0.0f));
	plane.SetScale(XMFLOAT3(1000.0f, 1000.0f, 1000.0f));
	meshes.push_back(plane);

	skySphere = Mesh(
			fbxNameSphere, 
			L"Shaders//SkyboxVS.fx", 
			L"Shaders//SkyboxPS.fx"
	);
	skySphere.AddTexture(cubeMap);
	skySphere.SetScale(XMFLOAT3(100.0f, 100.0f, 100.0f));
}
...
bool DXApp::ProcessMeshInit(Mesh * mesh)
{
	...
	// 래스터라이저 스테이트 생성.
	if (!mesh->CreateRasterizerState(
		pDevice, D3D11_FILL_SOLID, D3D11_CULL_NONE))
		return false;
	// 래스터라이저 스테이트 설정.
