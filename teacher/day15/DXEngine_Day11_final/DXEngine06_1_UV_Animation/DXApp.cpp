...
bool DXApp::InitTransformation()
{
	...
	// Test.
	CBPerFrame cDataFrame;
	ZeroMemory(&cDataFrame, sizeof(CBPerFrame));
	LONGLONG tick = GetTickCount();
	cDataFrame.time = (float)(tick / 1000.0f * 0.25f);
	cDataFrame.tickCount = (float)GetTickCount();

	// 버퍼 설정.
	D3D11_BUFFER_DESC cbDescFrame;
	ZeroMemory(&cbDescFrame, sizeof(D3D11_BUFFER_DESC));
	cbDescFrame.ByteWidth = sizeof(CBPerFrame);
	...
	D3D11_SUBRESOURCE_DATA cbDataFrame;
	ZeroMemory(&cbDataFrame, sizeof(D3D11_SUBRESOURCE_DATA));
	cbDataFrame.pSysMem = &cDataFrame;

	// 버퍼 생성.
	hr = pDevice->CreateBuffer(&cbDescFrame, &cbDataFrame, &cBufferFrame);
	if (FAILED(hr))
	{
		MessageBox(NULL, L"상수 버퍼 생성 실패.", L"오류", MB_OK);
		return false;
