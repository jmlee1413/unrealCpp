...
bool DXApp::InitScene()
{
	...
	for (int ix = 0; ix < meshes.size(); ++ix)
	{
		...
		if (!mesh->CreateRasterizerState(
			pDevice, D3D11_FILL_SOLID, D3D11_CULL_BACK))
			return false;
...
void DXApp::InitMeshInfo()
{
	...
	Mesh sphere(fbxNameSphere, L"Shaders//CubeVS.fx", L"Shaders//CubePS.fx");
	sphere.AddTexture(cubeMap);
	sphere.SetPosition(XMFLOAT3(0.0f, 0.0f, 0.0f));
	sphere.SetScale(XMFLOAT3(100.0f, 100.0f, 100.0f));
