...
DXApp::DXApp(HINSTANCE hinstance)
{
	...
	clientWidth = 1200;
	clientHeight = 1000;
	applicationName = L"Engine21 - Sky Light";
...
bool DXApp::InitScene()
{
	...
	if (!ProcessMeshInit(&skySphere))
		return false;

	if (!ProcessMeshInit(&fbxPlane))
		return false;

	for (int ix = 0; ix < meshes.size(); ++ix)
	{
		Mesh* mesh = &meshes[ix];
		if (!ProcessMeshInit(mesh))
			return false;
	}
	SetViewport();
...
void DXApp::InitMeshInfo()
{
	...
	Texture planeMap;
	planeMap.fileName = L"Resources//Textures//T_Ground_Grass_D.png";
	...
	// sky sphere
	LPCSTR fbxNameSphere = "Resources//Models//sphere.fbx";
	skySphere = Mesh(fbxNameSphere, L"Shaders//SkyBoxVS.fx", L"Shaders//SkyBoxPS.fx");
	skySphere.AddTexture(cubeMap);
	skySphere.SetScale(XMFLOAT3(1000.f, 1000.f, 1000.f));

	LPCSTR fbxNamePlane = "Resources//Models//plane.fbx";
	fbxPlane = Mesh(fbxNamePlane, L"Shaders//DiffuseVS.fx", L"Shaders//DiffusePS.fx");
	fbxPlane.AddTexture(planeMap);
	fbxPlane.SetPosition(XMFLOAT3(0.f, -90.f, 0.f));
	fbxPlane.SetRotation(XMFLOAT3(90.f, 0.f, 0.f));
	fbxPlane.SetScale(XMFLOAT3(200.f, 200.f, 200.f));

}

bool DXApp::ProcessMeshInit(Mesh* mesh)
{
	...
	// rasterizer state per mesh
	if (!mesh->CreateRasterizerState(pDevice,
...
{
	// 카메라 위치
	camera->SetPosition(XMVectorSet(0.f, 0.f, -200.f, 0.f));
	camera->SetFovY(75.f);
	...
	//행렬 설정
	mesh->SetWVPMatrices(mesh->GetWorldMatrix(), 
...
void DXApp::UpdateWVPBuffer(Mesh * mesh, CameraClass* camera)
{
	//InitWorldMatrix(mesh);
	mesh->SetWVPMatrices(mesh->GetWorldMatrix(), 
		camera->GetViewMatrix(), camera->GetProjectionMatrix());
