...
DXApp::DXApp(HINSTANCE hinstance)
{
	...
	clientWidth = 1000;
	clientHeight = 1000;
	applicationName = L"Engine16 - Cube Map";
...
bool DXApp::InitScene()
{
	...
	HRESULT hr;
	hr = D3DX11CompileFromFile(L"CubeVS.fx", NULL, NULL, "main",
	...
	hr = D3DX11CompileFromFile(L"CubePS.fx", NULL, NULL, "main",
...
bool DXApp::InitTransformation()
{
	...
	cameraPos    = XMVectorSet(0.f, 0.f, -250.f, 0.f);
...
bool DXApp::InitTexture()
{
	HRESULT hr = D3DX11CreateShaderResourceViewFromFile(
		pDevice, L"Yokohama3.dds", NULL, NULL, &pTexture, NULL);
	...
	if (FAILED(hr))
	{
		MessageBox(NULL, L"노멀맵 로드 실패", L"오류", MB_OK);
		return false;
	}
	...
	pDeviceContext->PSSetShaderResources(0, 1, &pTexture); // 텍스쳐
	pDeviceContext->PSSetShaderResources(1, 1, &pNormalTexture); // 노멀맵
	// ( 0, 2, []배열 ) 한줄로 줄일수 있음 배열로넣는게 맞다함
	...
	for (int ix = 0; ix < childCount; ++ix)
	{
		...
		for (int jx = 0; jx < polyCount; ++jx)
		{
			...
			for (int kx = 0; kx < vertexCount; ++kx)
			{
				...
				if (hasNoTangent)
				{
					MessageBox(NULL, L"탄젠트 없음", L"error", MB_OK);
				}
				vertex.tangent = hasNoTangent ? XMFLOAT3(0.f, 0.f, 0.f) :
					ReadTangent(fbxMesh, vertexIndex, vertexCounter);

				// 바이노멀 읽기
				bool hasNoBinormal = fbxMesh->GetElementBinormalCount() < 1;
				if (hasNoBinormal)
				{
					MessageBox(NULL, L"바이노멀 없음", L"error", MB_OK);
				}
				vertex.binormal = hasNoBinormal ? XMFLOAT3(0.f, 0.f, 0.f) :
