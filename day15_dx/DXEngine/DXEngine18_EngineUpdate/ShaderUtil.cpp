...
namespace ShaderUtil
{
	...
	// pixel //
	bool CreatePixelShader(ID3D11Device * device, const void * pointer, SIZE_T size, 
		ID3D11ClassLinkage * linkage, ID3D11PixelShader ** ppPixelShader)
	{
		HRESULT hr = device->CreatePixelShader(pointer, size, linkage, ppPixelShader);
		if (FAILED(hr))
		{
			MessageBox(NULL, L"pixel shader create failed", L"error", MB_OK);
			return false;
		}
